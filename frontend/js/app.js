...
document.addEventListener('alpine:init', () => {
    Alpine.data('app', () => ({
        ...
        async init() {
            try {
                ...
                window.addEventListener('auth:expired', async (event) => {
                    console.log('收到令牌过期事件:', event.detail);
                    await this.handleAuthExpired(event.detail?.message);
                });
        ...
        async checkAndRefreshToken() {
            ...
            // 检查令牌是否即将过期（5 分钟内）
            if (apiClient.isTokenExpiringSoon(5)) {
        ...
        // 处理认证过期（被动刷新）
        async handleAuthExpired(message) {
            // 先尝试刷新令牌
            ...
            this.showError(message || '登录已过期，请重新登录');
            // 显示登录模态框
        ...
        async checkAuth() {
            try {
                ...
                if (token && userId) {
                    ...
                    // 尝试获取当前用户信息
                    // 注意：如果 token 过期，api-client 会自动刷新并重试
                    try {
                        ...
                        // api-client 已经处理了 401 和 token 刷新
                        // 如果到这里还失败，说明刷新也失败了，auth 已被清除
                        // 此时应该初始化为未登录状态，但不创建新账户
                        this.isLoggedIn = false;
                        this.user = {};
                    }
                } else {
